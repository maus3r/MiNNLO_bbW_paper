...
#{{{ def: __init__(self,result_folder_path_in)
    def __init__(self,result_folder_path_in):
        ...
        self.gnuplot_folder_name = "gnuplot_LHEF_MATRIX_WP"
...
#        gnu.set_plot_properties("yunit","[fb]")
    ...
    all_pdfs = glob.glob("gnuplot_LHEF_MATRIX_WP/*.pdf")
    category_pdfs = list(sorted(glob.glob("gnuplot_LHEF_MATRIX_WP/*-*.pdf")))
    nocategory_pdfs = list(sorted(list(set(all_pdfs) - set(category_pdfs)),key=len))
    
    all_pdfs = nocategory_pdfs + category_pdfs

#    command = "pdfunite"
    command="gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=gnuplot_LHEF_MATRIX_WP/all_plots_vs_NNLO_WP.pdf "
    # Appending all pdfs
    for pdf in all_pdfs:
        command += " \"%s\"" % pdf
