...
#{{{ def: __init__(self,result_folder_path_in)
    def __init__(self,result_folder_path_in):
        ...
        self.gnuplot_folder_name = "gnuplot_LHEF_pt_vs_ptj_WP"
...
if __name__ == "__main__":
    ...
    for plot in all_plots:
        ...
        MiNNLOptj_lhef_kQ1   = MiNNLO_lhef_kQ1.replace("NLO_Wp_k","NLOptj_Wp_k")
        MiNLOptj_lhef_kQ1    = MiNLO_lhef_kQ1.replace("NLO_Wp_k","NLOptj_Wp_k")
        MiNNLOptj_lhef_kQ025 = MiNNLO_lhef_kQ025.replace("NLO_Wp_k","NLOptj_Wp_k")
        MiNLOptj_lhef_kQ025  = MiNLO_lhef_kQ025.replace("NLO_Wp_k","NLOptj_Wp_k")
        ...
        if os.path.exists(MiNNLO_lhef_kQ05):
            gnu.add_curve(MiNNLO_lhef_kQ05,{"format" : "histogram", "label" : "MiNNLO-pt (LHE)","line_style" : 1})
        if os.path.exists(MiNNLOptj_lhef_kQ05):
            gnu.add_curve(MiNNLOptj_lhef_kQ05,{"format" : "histogram", "label" : "MiNNLO-ptj (LHE)","line_style" : 4})
        ...
        if os.path.exists(MiNLO_lhef_kQ05):
            gnu.add_curve(MiNLO_lhef_kQ05,{"format" : "histogram", "label" : "MiNLO-pt (LHE)","line_style" : 2})
        if os.path.exists(MiNLOptj_lhef_kQ05):
            gnu.add_curve(MiNLOptj_lhef_kQ05,{"format" : "histogram", "label" : "MiNLO-ptj (LHE)","line_style" : 5})
...
#        gnu.set_plot_properties("yunit","[fb]")
    ...
    all_pdfs = glob.glob("gnuplot_LHEF_pt_vs_ptj_WP/*.pdf")
    category_pdfs = list(sorted(glob.glob("gnuplot_LHEF_pt_vs_ptj_WP/*-*.pdf")))
    nocategory_pdfs = list(sorted(list(set(all_pdfs) - set(category_pdfs)),key=len))
    
    all_pdfs = nocategory_pdfs + category_pdfs

#    command = "pdfunite"
    command="gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=gnuplot_LHEF_pt_vs_ptj_WP/all_plots_pt_vs_ptj_WP.pdf "
    # Appending all pdfs
    for pdf in all_pdfs:
        command += " \"%s\"" % pdf
